...
function App() {
  ...
  const handleAddTodo = () => {
    ...
    let updtesTodoArr = [...allTodos];
    updtesTodoArr.push(newTodoItem);
    setTodos(updtesTodoArr)
    localStorage.setItem('todolist', JSON.stringify(updtesTodoArr))
    setNewTitle("")
    setNewDescription("")
  }
    ...
    <div className="App">
      ...
      <div className="todo-wrapper">
        <div className="todo-input">
          <div className="todo-input_item">
            <label>Title</label>
            <input type="text" value={newTitle} onChange={(e) => setNewTitle(e.target.value)} placeholder='write here task title' />
          </div>
          <div className="todo-input_item">
            <label>Description</label>
            <input type="text" value={newDescription} onChange={(e) => setNewDescription(e.target.value)} placeholder='write here task Description' />
          </div>
          <div className="todo-input_item">
        ...
        <div className="todo_list">
          {isCompleteScreen === false && allTodos?.map((item, index) => {
            if (currentEdit === index) {
              ...
              return (
                <div className="todo-list_item" key={index}>
                  <div>
                    <h3>{item.title}</h3>
